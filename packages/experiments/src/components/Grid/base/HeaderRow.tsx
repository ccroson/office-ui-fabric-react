...
export class HeaderRow extends BaseComponent<IHeaderRowProps, {}> {
  ...
  public componentDidUpdate(prevProps: IHeaderRowProps, prevState: {}): void {
    if (prevProps.dirtyCanary !== this.props.dirtyCanary || prevProps.isHeaderFixed !== this.props.isHeaderFixed) {
      this.innerDivClientRect = this.innerDiv.getBoundingClientRect();
    }
  }

  protected renderComponent(): JSX.Element {
    const { children, height, theme } = this.props;
    ...
    return (
      <div className="grid-header-row" role="row" style={style} ref={this.resolveRef(this, 'innerDiv')}>
        {children}
      </div>
    );
  }
