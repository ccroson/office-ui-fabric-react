...
export class BaseGrid extends BaseComponent<IBaseGridProps, IBaseGridState> {
        ...
        if (this.props.selectionState) {
            ...
            if (newPrimaryCell) {
                if (newPrimaryCell.rowIndex !== previousPrimaryCell.rowIndex) {
                    this.verticalScrollToPrimaryCellIfNeeded(this.gridContainerRef);
                }
                if (newPrimaryCell.columnIndex !== previousPrimaryCell.columnIndex) {
                    this.horizontalScrollToPrimaryCellIfNeeded(this.gridContainerRef);
                }
            }

            // If switching from Edit to Select mode, set the focus back to the grid
            if (this.props.selectionState.mode === GridMode.Select && prevProps.selectionState && prevProps.selectionState.mode === GridMode.Edit) {
    ...
    private renderBody(): JSX.Element {
        ...
        if (selectionState.mode === GridMode.Edit && (selectionState.primaryCell.rowIndex < visibleStart || selectionState.primaryCell.rowIndex > visibleEnd)) {
     ...
     */
    private renderRow(activeRows: _.Dictionary<boolean>, rowIndex: number, selectedRowIndexes: _.Dictionary<boolean>, fixedOffScreen: boolean = false): JSX.Element {
        const {
    ...
    private updateGridLayoutProperties() {
        ...
        if (this.scrollableParent) {
            ...
            this.gridContainerInitialOffsetTopFromScrollableParent = this.gridContainerClientRect.top - this.scrollableParentClientRect.top + this.scrollTopPosition;
     ...
     * @param element The element to focus
     */
    private verticalScrollToPrimaryCellIfNeeded(element: HTMLElement) {
        const {
            headerRowHeight = GridDefaultProps.HeaderRowHeight,
            selectionState
        } = this.props;
        ...
        if (selectionState.mode === GridMode.Select && this.scrollableParent && this.gridContainerRef) {
            ...
            if (selectionState.primaryCell.rowIndex < rowRange.start) {
                ...
                this.scrollableParent.scrollTop = rowBounds.verticalOffset + gridOffsetTop + headerRowHeight - (isHeaderFixed ? headerRowHeight : 0);
                ...
                this.scrollableParent.scrollTop = rowBounds.verticalOffset + gridOffsetTop + headerRowHeight - this.scrollableParentClientHeight + rowBounds.height;
            }
     ...
     * @param element The element to focus
     */
    private horizontalScrollToPrimaryCellIfNeeded(element: HTMLElement): void {
        const {
            selectionState
        } = this.props;
        ...
        if (selectionState.mode === GridMode.Select && this.scrollableParent && this.gridContainerRef) {
            ...
            if (cellStart < gridVisibleStart) {
                ...
                this.scrollableParent.scrollLeft = RtlUtils.getBrowserSpecificScrollLeftValueWithOffset(this.scrollableParent, cellEnd - gridVisibleEnd);
            }
