...
import { css, getNativeProps } from '../../../Utilities';
...
export class Cell extends BaseComponent<ICellProps, {}> {
  ...
  protected renderComponent(): JSX.Element {
    ...
    let { ariaAndDataAttributes } = this.props;

    const borderClassNameMapping: CellClassMapping = this.getBorderClassName(
      fillPosition,
      selectionPosition,
      editing,
      isPrimary,
    ...
    return (
      ...
      >
        ...
        {children && <div className="grid-cell-content">{children}</div>}
  ...
  private getCellStyle(
    ...
    width: number,
    theme: GridTheme,
    cellClassMapping: CellClassMapping
  ): React.CSSProperties {
    ...
    if (rowSpan !== 0) {
      if (cellClassMapping.editing) {
        ...
        RtlUtils.setRTLSafeBorderLeft(
          style,
          null,
          null,
          cellClassMapping.selectionEdgeLeft ? theme.selectionBorderColor : theme.borderColor
        );

        RtlUtils.setRTLSafeBorderRight(
          style,
          null,
          null,
          cellClassMapping.selectionEdgeRight ? theme.selectionBorderColor : theme.borderColor
        );

        style.borderTopColor = cellClassMapping.selectionEdgeTop ? theme.selectionBorderColor : theme.borderColor;

        style.borderBottomColor = cellClassMapping.selectionEdgeBottom ? theme.selectionBorderColor : theme.borderColor;
