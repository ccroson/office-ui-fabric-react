...
export class DateEditor extends BaseComponent<IDateEditorProps, IDateEditorState> {
  ...
  protected renderComponent(): JSX.Element {
    const { isTextEditing } = this.props;

    const { isCalendarOpen = false } = this.state;
    ...
    return (
      <div ref={this.resolveRef(this, 'dateEditorContainer')} className="date-editor-container">
        {content}
        <i
          className={css('ms-Icon ms-Icon--Calendar', { 'calendar-open': isCalendarOpen })}
          onMouseDown={(event: React.MouseEvent<HTMLElement>) => {
      ...
      return (
        ...
        >
          <Calendar
            ...
            strings={
              new DateStrings(this.props.todayLabel, this.props.prevMonthAriaLabel, this.props.nextMonthAriaLabel)
            }
          />
        </Callout>
      );
  ...
  private onCalendarIconClick(event: React.MouseEvent<HTMLElement>): void {
    const { onCalendarIconClicked } = this.props;
  ...
  private getInputValue(): string {
    const { dateFormat = DateFormat.GridDate, value } = this.props;
  ...
  private onBlur(event: React.FocusEvent<HTMLElement>) {
    const { onEditConfirmed, value } = this.props;
