...
export class EnumEditor extends BaseComponent<IEnumEditorProps, IEnumEditorState> {
  ...
  public componentDidUpdate(prevProps: IEnumEditorProps, prevState: IEnumEditorState): void {
    const { pendingValue, onEditCancelled, onEditConfirmed } = this.props;
  ...
  protected renderComponent(): JSX.Element {
    ...
    let iconStyle: React.CSSProperties = this.props.theme
      ...
      : {};
    return (
      <div onClick={this.onCellClick} className="enum-editor" ref={this.resolveRef(this, 'EnumEditorContainerRef')}>
        <div
          className="enum-editor-value"
          onKeyDown={this.onKeyDown}
          onKeyPress={this.onKeyPress}
          onMouseDown={(event: React.MouseEvent<HTMLElement | HTMLInputElement>) => {
            /*is this needed?*/ event.stopPropagation();
          }}
        >
          {val}
        </div>
  ...
  private renderCallout(): JSX.Element {
    const { enumOptions, forceCalloutOpen, value, width } = this.props;
    const { calloutOpened = false } = this.state;
    return (
      (calloutOpened || forceCalloutOpen) && (
        ...
        >
          <div className="enum-editor-options" style={{ width: width }}>
            <FocusZone
                ...
                return (
                  <div
                    ...
                    onClick={(event: React.MouseEvent<HTMLElement>) => {
                      this.onOptionSelected(event, option.key);
                    }}
  ...
  private getDisplayedValue(): string {
    const { enumOptions, value, pendingValue } = this.props;
    if (pendingValue) {
      let found: EnumOption = this.getOptionFromPending();
      if (found) {
        return found.text;
    ...
    return (
      value &&
      _.find(enumOptions, (option: EnumOption) => {
        return option.key === value.selectedEnumKey;
      }).text
    );
  }
  ...
  private getOptionFromPending(): EnumOption {
    const { enumOptions, pendingValue } = this.props;
    if (pendingValue) {
      if (_.isString(pendingValue)) {
        return _.find(enumOptions, (option: EnumOption) => {
          return option.text.charAt(0).toLowerCase() === pendingValue.toLowerCase();
        });
      } else {
        return _.find(enumOptions, (option: EnumOption) => {
          return option.key === pendingValue.selectedEnumKey;
        });
      }
    }
    return null;
  ...
  private onTryUpdateSelected(character: string): void {
    const { enumOptions, onValueUpdated, value } = this.props;
    let val: EnumOption = _.find(enumOptions, (option: EnumOption) => {
      return option.text.charAt(0).toLowerCase() === character.charAt(0).toLowerCase();
    });
    if (val) {
      let newValue = value;
  ...
  private onOptionSelected(event: React.MouseEvent<HTMLElement>, optionKey: EnumKey): void {
    const { onEditConfirmed } = this.props;
    event.stopPropagation();
