...
export class Grid extends AbstractGrid<IGridProps, IGridState> implements IGrid {
    ...
    public changePrimaryCell(primaryCell: GridCoordinate): void {
        ...
        let newSelectionState: SelectionState = primaryCell != null ?
            {
                ...selectionState,
    ...
    public changeSelection(selections: GridRegion[]): void {
        ...
        let newSelectionState: SelectionState = selections != null && selections.length > 0 ?
            {
                ...selectionState,
    ...
    protected renderValidationError(): JSX.Element {
        ...
        if (validationError) {
            ...
            return (
                ...
                >
                    <div
                        className="validation-callout-content"
                    >
                        <i className="ms-Icon ms-Icon--ErrorBadge"></i>
                        <div className="validation-message" role="alert" aria-live="assertive" aria-atomic="true">{ validationError.errorMessage }</div>
                    </div>
                </Callout>
            );
        }
    ...
    protected getRenderedElement(cellCoordinate: GridCoordinate, extractedCellData: any, columnWidth: number): JSX.Element | string {
        ...
        let shouldRenderEditor: boolean = selectionState.mode === GridMode.Edit &&
            selectionState.primaryCell.equals(cellCoordinate) &&
            cellDefinition.type.renderEditor != null;

        let shouldRenderSelected: boolean = (selectionState.mode === GridMode.Select || selectionState.mode === GridMode.Selecting) &&
            selectionState.primaryCell.equals(cellCoordinate) &&
    ...
    protected onCellMouseUp(event: React.MouseEvent<HTMLElement>): void {
        ...
        this.updateFromMouseEvent(event, this.stateManager.handleCellMouseUp(selectionState, this.transitionToEditModeAfterSelecting));
        this.transitionToEditModeAfterSelecting = false;
    }
    ...
    protected onRowHeaderMouseEnter(rowIndex: number, event: React.MouseEvent<HTMLElement>): void {
        ...
        this.updateFromMouseEvent(event, this.stateManager.handleCellMouseEnter(selectionState, new GridCoordinate(rowIndex, 0)));
    }
    ...
    protected onFillCompleted(sourceRegion: GridRegion, targetRegion: GridRegion): void {
        ...
        if (sourceRegion && targetRegion && onDataUpdated) {
            ...
            for (let rowIndex = targetRegion.rowRange.start; rowIndex <= targetRegion.rowRange.end; rowIndex++) {
                ...
                for (let columnIndex = sourceRegion.columnRange.start; columnIndex <= sourceRegion.columnRange.end; columnIndex++) {
     ...
     */
    private getSourceRowForFill(sourceRows: Object[], targetRowIndex: number, sourceRegion: GridRegion, targetRegion: GridRegion): Object {
        ...
        if (isFillDown) {
            ...
            return sourceRows[(sourceRegion.rowRange.end - (targetRegion.rowRange.end - targetRowIndex) % sourceRows.length - sourceRegion.rowRange.start) % sourceRows.length];
        }
    }
    ...
    private shouldAddFooterRow(props: IGridProps = this.props): boolean {
        const {
            isReadOnlyOverride,
            showFooterRow = GridDefaultProps.ShowFooterRow
        } = props;
        ...
        try {
            if (onDataUpdated) {
                ...
                let cleanedUpdates: DataUpdateInternal[] = _.map(dataUpdates, (dataUpdate: DataUpdateInternal) => {
                    ...
                    dataUpdate.updatedValue = columnDefinition.cell.type.parseRawInput ? columnDefinition.cell.type.parseRawInput(originalData, dataUpdate.updatedValue) : dataUpdate.updatedValue;
                    return dataUpdate;
                });
                // first, validate each update
                let validationResults: __ValidationResult[] = this.validateUpdates(cleanedUpdates);
                let invalidUpdates = _.filter(validationResults, (validationResult: __ValidationResult) => !validationResult.isValid);
    ...
    private sendUpdates(dataUpdates: DataUpdateInternal[]): void {
        ...
        if (onDataUpdated) {
            ...
            let groupedUpdates = _.groupBy(dataUpdates, (dataUpdateInternal: DataUpdateInternal) => dataUpdateInternal.cellCoordinate.rowIndex);
    ...
    private onSelectionStateChanged(selectionState: SelectionState): void {
        ...
        if (selectionState) {
            ...
            let handleStateChange = () => {
                if (onPrimaryCellChanged && selectionState.primaryCell && !selectionState.primaryCell.equals(priorPrimaryCell)) {
                    onPrimaryCellChanged(this.getRowDataAtIndex(selectionState.primaryCell.rowIndex), this.getColumnDefinitionAtIndex(selectionState.primaryCell.columnIndex));
                }
     ...
     * @param newState The new selection state acquired from a state manager
     */
    private updateFromKeyboardEvent(
        event: React.SyntheticEvent<HTMLElement>,
        newState: SelectionState
    ): void {
     ...
     */
    private initializeStateManager(selectionMode: SelectionMode, hideColumnHeader: boolean = GridDefaultProps.HideColumnHeader): StateManager {
