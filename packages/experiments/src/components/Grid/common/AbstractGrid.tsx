...
import { ContextualMenu, IContextualMenuItem, DirectionalHint } from "office-ui-fabric-react/lib-commonjs/ContextualMenu";
 ...
 */
export abstract class AbstractGrid<P extends IAbstractGridProps, S extends IAbstractGridState> extends BaseComponent<P, S> {
    /** The BaseGrid ref */
     ...
     */
    protected abstract getRenderedElement(cellCoordinate: GridCoordinate, extractedCellData: any, columnWidth: number): JSX.Element | string;
    ...
    protected renderColumnHeaderCell(columnIndex: number): JSX.Element {
        ...
        return (
            <div className="grid-column-header-cell-content-parts">
                <div>
                    { arrowClassMapping &&
                        <i className={ css("ms-Icon", arrowClassMapping) }></i> }
                    { column.header.label }
                </div>
                {/* Render the context menu icon if enabled */
                    ...
                    <div
                        ...
                        role="menuitem"
                    >
                        <i className={ `ms-Icon ms-Icon--${Icons.ChevronDownMed}` }></i>
                    </div>
                }
            </div>
        );
     ...
     */
    protected renderHeaderContextMenu(columnDefinitions: IColumnDefinition[], headerContextMenuIndex: number): JSX.Element {
    ...
    protected getCellContextMenuItemsFromCoordinate(cellCoordinate: GridCoordinate): IContextualMenuItem[] {
        ...
        const contextMenuItems: IContextualMenuItem[] = PropUtils.getValueFromAccessor(columnDefinition.cell.contextMenuItems, row);
    ...
    protected onColumnHeaderAltDown(columnIndex: number, event: React.KeyboardEvent<HTMLElement>): void {
        ...
        if (this.getHeaderContextMenuItems(columnDefinition).length > 0) {
            ...
            const headerCell = event.currentTarget.getElementsByClassName("grid-column-header-cell-content-parts").item(columnIndex) as HTMLElement;
            this.headerContextMenuTarget = (headerCell ? headerCell.getElementsByClassName("grid-column-header-context-menu-indicator").item(0) : null) as HTMLElement;

            while (this.headerContextMenuTarget && !this.headerContextMenuTarget.classList.contains('grid-column-header-cell')) {
                this.headerContextMenuTarget = this.headerContextMenuTarget.parentElement;
                // break out of loop if we hit the highest element the element we are looking for could be contained in
                if (this.headerContextMenuTarget === this.baseGrid.headerRowRef.innerDiv) {
                    return;
    ...
    protected getCellContext(cellCoordinate: GridCoordinate, columnWidth: number): CellContext {
        ...
        return {
            ...
            isSelected: (passedCellCoordinate: GridCoordinate) => GridUtilities.isCellInsideAnySelection(cellCoordinate, selectionState.selections),
            isSelectionSingleCell: () => selectionState.selections.length === 1 && selectionState.selections[0].isSingleCell(),
            inFooterRow: false,
            theme: this.gridTheme
        };
    }
    ...
    protected getIsColumnHeaderClickable(columnIndex: number): boolean {
        let columnDefinition: IColumnDefinition = this.getColumnDefinitionAtIndex(columnIndex);
        return columnDefinition.sortable || (columnDefinition.header.contextMenuItems && columnDefinition.header.contextMenuItems.length > 0);
    }
    ...
    private adjustSelectionsToFit() {
        ...
        if (selectionState && _.size(selectionState.selections)) {
            ...
            for (let i: number = selectionState.selections.length - 1; i >= 0; i--) {
                ...
                let primaryCoordinateRowIndex: number = currentRegion && currentRegion.primaryCoordinate && currentRegion.primaryCoordinate.rowIndex;
                let secondaryCoordinateRowIndex: number = currentRegion && currentRegion.secondaryCoordinate && currentRegion.secondaryCoordinate.rowIndex;
        ...
        if (columnDefinitions && columnDefinitions.length > 0) {
            _.forEach(columnDefinitions, (columnDefinition: IColumnDefinition) => {
                ...
                if (!columnDefinition.id) {
                    throw new PropValidationError(componentName, "columnDefinitions", "Column definition must have a non null, non empty, unique Id field");
                }
                ...
                if (columnDefinition.cell.accessor && columnDefinition.cell.property) {
                    throw new PropValidationError(
                        componentName,
                        "columnDefinition",
                        "Cell definition provides both accessor delegate and property name. Only one can be used at a time. Column definition: " + JSON.stringify(columnDefinition)
                    );
                }
                ...
                if (!columnDefinition.cell.accessor && !columnDefinition.cell.property) {
                    throw new PropValidationError(
                        componentName,
                        "columnDefinition",
                        "Cell definition must provide either an accessor delegate or a property name. Column Definition: " + JSON.stringify(columnDefinition)
                    );
                }
                ...
                if (columnDefinition.sortable && (!onSortByColumn || !onClearSort)) {
                    throw new PropValidationError(
                        componentName,
                        "columnDefinition",
                        "Column definition marked as sortable, but there are no props for onColumnSort and onColumnSortClear : " + JSON.stringify(columnDefinition)
                    );
                }
            ...
            if (_.uniqBy(columnDefinitions, (columnDefinition: IColumnDefinition) => columnDefinition.id).length !== columnDefinitions.length) {
                throw new PropValidationError(componentName, "columnDefinition", "Column definitions must contain unique ids");
            }

        } else {
            throw new PropValidationError(componentName, "columnDefinition", "ColumnDefinition list must not be null or empty");
        }
    }
    ...
    private updateColumnWidths(): void {
        if (this.gridWidthContainer) {
            ...
            let columnWidths: number[] = this.computeColumnWidths(
                _.map(this.columnDefinitions, (columnDefinition: IColumnDefinition) => GridSize.parseSize(columnDefinition.width)),
                availableWidth
            );
    ...
    private didColumnDefinitionsChange(nextProps: IAbstractGridProps): boolean {
        ...
        return _.find(nextProps.columnDefinitions, (columnDef: IColumnDefinition, index: number) => {
...
export interface ICellType {
     ...
     */
    render: (cellData: Object, context: CellContext) => JSX.Element | string;
