...
import { CellRegionPosition, GridCoordinate, RowRange } from './Common';
...
export class GridRegion {
  ...
  public fillPartialCells(
    ...
    // Bottom row
    let cellToMove: GridCoordinate =
      this._secondaryCoordinate.rowIndex === this.rowRange.end ? this._secondaryCoordinate : this._primaryCoordinate;
    ...
    if (!movedPastPrimary) {
      cellToMove =
        this._secondaryCoordinate.rowIndex === this.rowRange.start
          ? this._secondaryCoordinate
          : this._primaryCoordinate;
      do {
        foundPartialCell = false;
        for (let column = this.columnRange.start; column <= this.columnRange.end; column++) {
  ...
  public getFillRegion(cellCoordinate: GridCoordinate): GridRegion {
    ...
    if (rowRange.end < cellCoordinate.rowIndex) {
      // the cell is below the selection region, filling down
      return new GridRegion(
        new GridCoordinate(rowRange.end + 1, columnRange.start),
        new GridCoordinate(cellCoordinate.rowIndex, columnRange.end)
      );
    } else if (rowRange.start > cellCoordinate.rowIndex) {
      // the cell is above the selection region, filling up
      return new GridRegion(
        new GridCoordinate(rowRange.start - 1, columnRange.start),
        new GridCoordinate(cellCoordinate.rowIndex, columnRange.end)
      );
    }
  ...
  public isCellInRegion(cellCoordinate: GridCoordinate): boolean {
    ...
    return (
      rowRange.start <= cellCoordinate.rowIndex &&
  ...
  public equals(other: GridRegion): boolean {
    return other != null && _.isEqual(other.rowRange, this.rowRange) && _.isEqual(other.columnRange, this.columnRange);
  }
