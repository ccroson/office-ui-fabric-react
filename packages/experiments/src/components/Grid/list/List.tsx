...
// Utilities
import { autobind, css, IDictionary } from '../../../Utilities';
...
export class List extends AbstractGrid<IListProps, IListState> {
  ...
  constructor(props: IListProps, context?: any) {
    ...
    const { selectionMode = ListSelectionMode.None, hideColumnHeader } = this.props;
  ...
  protected get selectionMode(): SelectionMode {
    const { selectionMode = ListSelectionMode.None } = this.props;
  ...
  public componentWillReceiveProps(nextProps: IListProps) {
    ...
    const { selectionMode = ListSelectionMode.None, columnDefinitions, rowData, rowKey, hideColumnHeader } = nextProps;
  ...
  protected getRenderedElement(
    ...
    if (
      selectionState.mode === GridMode.Select &&
      selectionState.primaryCell.equals(cellCoordinate) &&
      cellDefinition.type.renderSelected
    ) {
  ...
  protected onSelectionStateChanged(selectionState: SelectionState): void {
    const { onSelectionChanged, rowKey } = this.props;
   ...
   */
  private updateFromEvent(event: React.SyntheticEvent<HTMLElement>, newState: SelectionState): void {
    if (newState) {
  ...
  private initializeStateManager(
    ...
    return StateManagerFactory.createStateManager(gridSelectionMode, {
  ...
  private toggleSelection(rowIndex: number): void {
    ...
    const { onSelectionChanged, selectionMode = ListSelectionMode.None } = this.props;
    ...
    if (onSelectionChanged) {
      let selectedRows: Object[] = _.map(selectedRowIndexes, (selectedRowIndex: number) =>
        this.getRowDataAtIndex(selectedRowIndex)
      );
      onSelectionChanged(selectedRows);
    }
  }
   ...
   */
  private getSelectionsFromMapping(
    rowData: Object[],
    rowKey: (data: Object) => string,
    mapping: _.Dictionary<boolean>
  ): GridRegion[] {
      ...
      for (let i = 0; i < rowData.length; i++) {
        ...
        if (this.selectedKeys[resolvedRowKey]) {
          selectedRegions.push(
            new GridRegion(new GridCoordinate(i, 0), new GridCoordinate(i, this.getMaxColumnIndex()))
          );
        }
      }
    }
