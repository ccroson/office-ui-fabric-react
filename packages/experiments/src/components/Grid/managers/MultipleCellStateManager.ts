...
import { StateManager, SelectionState, StateManagerParameters } from './StateManager';
...
export class MultipleCellStateManager extends StateManager {
  ...
  public handleEnter(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select || mode === GridMode.Edit) {
      ...
      if (primaryCell.rowIndex + (rowSpan - 1) < this.getMaxRowIndex()) {
        let newPrimaryCell: GridCoordinate = new GridCoordinate(
          primaryCell.rowIndex + rowSpan,
          primaryCell.columnIndex
        );
        return {
          ...prevState,
          mode: GridMode.Select,
  ...
  public handleHome(prevState: SelectionState): SelectionState {
    const { mode, primaryCell, selections } = prevState;
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      ...
      if (minSelectableColumnIndex !== -1) {
        ...
        if (!alreadyHome || !selection.isSingleCell()) {
          const newPrimaryCell: GridCoordinate = this.getMappedCell(
            new GridCoordinate(primaryCell.rowIndex, minSelectableColumnIndex)
          );
  ...
  public handleShiftHome(prevState: SelectionState): SelectionState {
    const { mode, selections } = prevState;

    if (mode === GridMode.Select && selections.length === 1) {
      const minSelectableColumnIndex: number = this.getMinSelectableColumnIndex();
      let selection: GridRegion = this.getPrimarySelection(prevState);
      if (minSelectableColumnIndex !== -1 && selection.secondaryCoordinate.columnIndex !== minSelectableColumnIndex) {
        const newSecondaryCoordinate: GridCoordinate = new GridCoordinate(
          selection.secondaryCoordinate.rowIndex,
          minSelectableColumnIndex
        );
  ...
  public handleControlShiftHome(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      ...
      if (
        minSelectableColumnIndex !== -1 &&
        (secondaryCoordinate.rowIndex !== 0 || secondaryCoordinate.columnIndex !== minSelectableColumnIndex)
      ) {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      ...
      if (maxSelectableColumnIndex !== -1) {
        ...
        if (!alreadyEnd || !selection.isSingleCell()) {
          const newPrimaryCell: GridCoordinate = this.getMappedCell(
            new GridCoordinate(primaryCell.rowIndex, maxSelectableColumnIndex)
          );
  ...
  public handleControlEnd(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      ...
      if (maxSelectableColumnIndex !== -1) {
        ...
        const alreadyEnd: boolean =
          primaryCell.rowIndex === maxRowIndex && primaryCell.columnIndex === maxSelectableColumnIndex;
        let selection: GridRegion = this.getPrimarySelection(prevState);
  ...
  public handleControlShiftEnd(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      ...
      if (
        maxSelectableColumnIndex !== -1 &&
        (secondaryCoordinate.rowIndex !== maxRowIndex || secondaryCoordinate.columnIndex !== maxSelectableColumnIndex)
      ) {
  ...
  public handleShiftLeft(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      let selection: GridRegion = this.getPrimarySelection(prevState);
      if (
        selection.secondaryCoordinate.columnIndex > 0 &&
        this.isColumnSelectable(selection.secondaryCoordinate.columnIndex - 1)
      ) {
        let newSecondaryCoordinate: GridCoordinate = new GridCoordinate(
          selection.secondaryCoordinate.rowIndex,
          selection.secondaryCoordinate.columnIndex - 1
        );
  ...
  public handleShiftRight(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1) {
      let selection: GridRegion = this.getPrimarySelection(prevState);
      if (
        selection.secondaryCoordinate.columnIndex < this.getMaxColumnIndex() &&
        this.isColumnSelectable(selection.secondaryCoordinate.columnIndex + 1)
      ) {
        let newSecondaryCoordinate: GridCoordinate = new GridCoordinate(
          selection.secondaryCoordinate.rowIndex,
  ...
  public handleDown(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select) {
      const rowSpan = this.getRowSpan(primaryCell);
      if (primaryCell.rowIndex + (rowSpan - 1) < this.getMaxRowIndex()) {
        const newPrimaryCell: GridCoordinate = new GridCoordinate(
          primaryCell.rowIndex + rowSpan,
          primaryCell.columnIndex
        );
  ...
  public handleShiftDown(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select && selections.length === 1 && !prevState.primaryCell.isColumnHeaderCell) {
      ...
      if (selection.secondaryCoordinate.rowIndex < this.getMaxRowIndex()) {
        let newSecondaryCoordinate: GridCoordinate = new GridCoordinate(
          selection.secondaryCoordinate.rowIndex + 1,
          selection.secondaryCoordinate.columnIndex
        );
  ...
  public handleUp(prevState: SelectionState): SelectionState {
    ...
    if (mode === GridMode.Select) {
      ...
      if (primaryCell.rowIndex === 0) {
        ...
        newPrimaryCell = this.getMappedCell(new GridCoordinate(primaryCell.rowIndex - 1, primaryCell.columnIndex));
      } else {
        return; //No action
      }
  ...
  public handleCellMouseDown(prevState: SelectionState, target: GridCoordinate): SelectionState {
    ...
    if (target.isColumnHeaderCell) {
      newPrimaryCell = this.getMappedCell(new GridCoordinate(target.rowIndex - 1, target.columnIndex, true));

      mode = GridMode.Select;
    } else if (target.isRowHeaderCell) {
      newPrimaryCell = this.getMappedCell(new GridCoordinate(target.rowIndex, target.columnIndex, false, true));

      mode = GridMode.Selecting;
    } else if (this.isColumnSelectable(target.columnIndex)) {
      newPrimaryCell = this.getMappedCell(new GridCoordinate(target.rowIndex, target.columnIndex));
  ...
  private getPrimarySelection(selectionState: SelectionState) {
    const { primaryCell, selections } = selectionState;
  ...
  private expandPrimarySelection(prevState: SelectionState, target: GridCoordinate): GridRegion[] {
    const { primaryCell, selections } = prevState;

    // If this cell is mapped, get the mapped cell
    target = this.getMappedCell(target);

    let newSelection: GridRegion = this.getRectangularSelection(new GridRegion(primaryCell, target));

    // if the new selection causes overlap, ignore it, we don't want to have overlapping selections.
    let nonPrimarySelections: GridRegion[] = _.filter(
      selections,
      (selection: GridRegion) => !selection.isCellInRegion(primaryCell)
    );
