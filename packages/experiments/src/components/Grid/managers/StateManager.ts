    ...
    protected getNextTabCell(cell: GridCoordinate): GridCoordinate {
        ...
        if (cell.isColumnHeaderCell) {
            ...
            if (cell.columnIndex === maxSelectableColumnIndex && cell.rowIndex < maxRowIndex) {
                newCell = this.getMappedCell(
                    new GridCoordinate(cell.rowIndex + 1, minSelectableColumnIndex));
            } else if (cell.columnIndex < maxSelectableColumnIndex) {
                newCell = this.getMappedCell(
                    new GridCoordinate(cell.rowIndex, cell.columnIndex + 1));
            }
        }
        return newCell;
    }
    ...
    protected getPreviousTabCell(cell: GridCoordinate): GridCoordinate {
        ...
        if (cell.isColumnHeaderCell) {
            if (cell.columnIndex > minSelectableColumnIndex) {
                newCell = this.getMappedCell(
                    new GridCoordinate(cell.rowIndex, cell.columnIndex - 1, true));
            }
        } else {
            if (cell.rowIndex === minRowIndex) {
                if (cell.columnIndex === minSelectableColumnIndex) {
                    newCell = this.getMappedCell(
                        new GridCoordinate(cell.rowIndex - 1, maxSelectableColumnIndex, true));
                } else {
                    newCell = this.getMappedCell(
                        new GridCoordinate(cell.rowIndex, cell.columnIndex - 1));
                }
                ...
                if (cell.columnIndex === minSelectableColumnIndex) {
                    newCell = this.getMappedCell(
                        new GridCoordinate(cell.rowIndex - 1, maxSelectableColumnIndex));
                } else {
                    newCell = this.getMappedCell(
                        new GridCoordinate(cell.rowIndex, cell.columnIndex - 1));
                }
