...
export class EnumCell implements ICellType {
    ...
    public render(cellData: EnumCellData, cellContext: CellContext): JSX.Element | string {
        if (!cellData ||
            ...
            cellData.selectedEnumKey == null || cellData.selectedEnumKey.toString() === "") {
            return null;
        }
        let value = _.find(this.enumOptions, (option: EnumOption) => { return option.key === cellData.selectedEnumKey; }).text;
        return value;
    }
    ...
    public sortComparator(left: EnumCellData, right: EnumCellData): number {
        ...
        return StringUtils.stringCompareOrdinalCaseInsensitive(left.selectedEnumKey.toString(), right.selectedEnumKey.toString());
    }
    ...
    public parseRawInput(originalValue: EnumCellData, changedValue: string | EnumCellData | any): EnumKey {
        ...
        let updatedOption = _.find(this.enumOptions, (option: EnumOption) => { return option.key === changedValue; });
        ...
        if (!updatedOption &&
            ...
            updatedOption = _.find(this.enumOptions, (option: EnumOption) => { return option.text.toLowerCase().charAt(0) === changedValue.toLowerCase(); });
        }
        if (updatedOption) {
