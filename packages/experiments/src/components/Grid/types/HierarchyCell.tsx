...
import { css, IDictionary, getRTL } from '../../../Utilities';
...
export class HierarchyCell implements ICellType {
   ...
   */
  constructor(
    underlyingCellType: ICellType,
    onNodeCollapsed: (node: HierarchyCellData) => void,
    onNodeExpanded: (node: HierarchyCellData) => void
  ) {
  ...
  public parseRawInput(originalValue: HierarchyCellData, changedValue: any): Object {
    return this.underlyingCellType.parseRawInput
      ? this.underlyingCellType.parseRawInput(
          originalValue.cellData,
          changedValue != null && changedValue.cellData != null ? changedValue.cellData : changedValue
        )
      : changedValue;
  }
  ...
  private renderWrapper(
    ...
    return (
      <div
        ...
        style={
          getRTL()
            ? { paddingRight: cellData && cellData.indentationLevel + 'em' }
            : { paddingLeft: cellData && cellData.indentationLevel + 'em' }
        }
      >
        {this.renderTreeChevron(cellData, context)}
        <span className="wrapped-content">{children}</span>
      </div>
    );
  }
