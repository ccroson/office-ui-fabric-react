...
export namespace GridUtilities {
  ...
  /**
   * Gets a value from an object and a property string. The data can be any Javascript object and the property string can be any number of properties
   * separated by '.', such as 'prop1.prop2'
  ...
  export function getCellRegionPositionIfSelected(
    ...
    let containingSelection: GridRegion = _.find(selections, (selection: GridRegion) =>
      selection.isCellInRegion(cellCoordinate)
    );
